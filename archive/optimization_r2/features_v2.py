"""Advanced Feature Engineering (Round 2) for Dynamic Oracle.

Computes multi-scale form, opponent-adjusted dynamics, Elo velocities & trends,
clean sheets / FTS rates, goal volatility, Bayesian H2H draw affinities, and
contextual stakes.

All features are computed chronologically prior to kickoff (Zero Temporal Leakage).
"""
...
from src.optimization.features import _bivariate_poisson_probs


class AdvancedHistoryBufferV2:
    """Maintains rolling match history, Elo trajectories, H2H, and venue specialization."""
    ...
    def __init__(self, form_windows: list[int] = (3, 5, 8, 10, 15, 20, 30)):
        ...
        self._elo_history: dict[str, deque] = {}
        self._venue_history: dict[str, dict[str, deque]] = {}  # team -> {"home": deque, "away": deque}
    ...
    def get_dynamics(self, team: str) -> dict:
        """Compute advanced form dynamics: clean sheets, FTS, goal volatility, acceleration."""
        hist = self._history.get(team)
        if hist is None or len(hist) < 3:
            return {
                "clean_sheet_rate_10": np.nan,
                "failed_to_score_rate_10": np.nan,
                "gd_volatility_10": np.nan,
                "form_accel": np.nan,
                "elo_velocity_3": np.nan,
                "elo_velocity_5": np.nan,
                "elo_volatility_10": np.nan,
            }

        recent_10 = list(hist)[-min(len(hist), 10):]
        n10 = len(recent_10)
        clean_sheets = sum(1.0 for m in recent_10 if m["ga"] == 0) / n10
        fts = sum(1.0 for m in recent_10 if m["gf"] == 0) / n10
        gds = [m["gf"] - m["ga"] for m in recent_10]
        gd_vol = float(np.std(gds)) if len(gds) > 1 else 0.0

        # Form acceleration: pts_3 vs pts_10
        recent_3 = list(hist)[-min(len(hist), 3):]
        pts_3 = sum(3.0 if m["res"] == 1.0 else (1.0 if m["res"] == 0.5 else 0.0) for m in recent_3) / len(recent_3)
        pts_10 = sum(3.0 if m["res"] == 1.0 else (1.0 if m["res"] == 0.5 else 0.0) for m in recent_10) / n10
        form_accel = pts_3 - pts_10

        # Elo trajectories
        elo_h = self._elo_history.get(team, deque())
        if len(elo_h) >= 4:
            elo_list = list(elo_h)
            v3 = (elo_list[-1] - elo_list[-min(len(elo_list), 4)]) / 3.0
            v5 = (elo_list[-1] - elo_list[-min(len(elo_list), 6)]) / min(len(elo_list) - 1, 5.0)
            diffs = np.diff(elo_list[-min(len(elo_list), 10):])
            elo_vol = float(np.std(diffs)) if len(diffs) > 1 else 0.0
        else:
            v3 = 0.0
            v5 = 0.0
            elo_vol = 0.0
        ...
        return {
            "clean_sheet_rate_10": clean_sheets,
            "failed_to_score_rate_10": fts,
            "gd_volatility_10": gd_vol,
            "form_accel": form_accel,
            "elo_velocity_3": v3,
            "elo_velocity_5": v5,
            "elo_volatility_10": elo_vol,
        }
    ...
    def get_h2h(self, team_a: str, team_b: str) -> dict:
        ...
        if n_meetings == 0:
            return {
                "h2h_matches": 0.0,
                "h2h_win_rate_home": 0.38,
                "h2h_draw_rate": 0.25,
                "h2h_gd_home": 0.0,
                "h2h_recency_gd": 0.0,
                "h2h_draw_affinity": 0.25,
            }

        wins_a = 0.0
        draws = 0.0
        gd_a = 0.0
        recency_weights = np.exp(-0.15 * np.arange(n_meetings)[::-1])  # most recent has highest weight
        recency_weighted_gd = 0.0
        ...
        for i, m in enumerate(records):
            ...
            if m["home"] == team_a:
                ...
                g_diff = (m["ga"] - m["gf"])
            gd_a += g_diff
            recency_weighted_gd += w * g_diff

        recency_weighted_gd /= np.sum(recency_weights)
        ...
        return {
            ...
            "h2h_recency_gd": float(recency_weighted_gd),
            "h2h_draw_affinity": float(shrunk_draw),
        }
    ...
    def record_match(
        ...
        # 2. Update Elo history
        self._elo_history.setdefault(home, deque(maxlen=20)).append(elo_home)
        self._elo_history.setdefault(away, deque(maxlen=20)).append(elo_away)
...
def build_advanced_feature_matrix_v2(
    ...
    """Construct high-capacity Round 2 feature matrix with full domain feature engineering."""
    tracker = StrengthTracker(updater_cfg)
    buf = AdvancedHistoryBufferV2(form_windows=form_windows)
    rows = []
    ...
    for row in matches.itertuples(index=False):
        ...
        # 3. Match Context & Stakes Features
        ...
        # Stakes weight
        if feats["is_world_cup"] == 1.0 or feats["is_euro"] == 1.0:
            stakes = 1.5
        elif feats["is_friendly"] == 1.0:
            stakes = 0.8
        else:
            stakes = 1.1
        feats["competitive_stakes"] = stakes
        feats["neutral_elo_interaction"] = feats["is_neutral"] * raw_diff

        # Rest days & non-linear rest advantage
        hr = min(60.0, max(0.0, buf.rest_days(home, date)))
        ar = min(60.0, max(0.0, buf.rest_days(away, date)))
        feats["home_rest_days"] = hr
        feats["away_rest_days"] = ar
        feats["rest_diff"] = hr - ar
        feats["rest_advantage_flag"] = 1.0 if hr >= ar + 3.0 else (-1.0 if ar >= hr + 3.0 else 0.0)
        ...
        for team, prefix in ((home, "home"), (away, "away")):
            ...
            dyn = buf.get_dynamics(team)
            for k, v in dyn.items():
                feats[f"{prefix}_{k}"] = v

        # Form & Velocity differentials
        for w in (3, 5, 10, 20):
        ...
        feats["diff_form_accel"] = feats.get("home_form_accel", 0.0) - feats.get("away_form_accel", 0.0)
        feats["diff_elo_velocity_3"] = feats.get("home_elo_velocity_3", 0.0) - feats.get("away_elo_velocity_3", 0.0)
        feats["diff_elo_velocity_5"] = feats.get("home_elo_velocity_5", 0.0) - feats.get("away_elo_velocity_5", 0.0)
        feats["diff_clean_sheet_10"] = feats.get("home_clean_sheet_rate_10", 0.0) - feats.get("away_clean_sheet_rate_10", 0.0)
        feats["diff_fts_10"] = feats.get("home_failed_to_score_rate_10", 0.0) - feats.get("away_failed_to_score_rate_10", 0.0)
