"""Fast Monte Carlo Simulator for 2022 FIFA World Cup (Qatar) using our model."""
...
def run_2022_world_cup_simulation():
    oracle = load_oracle()
    engine = MatchEngine(MatchEngineConfig(home_advantage=0.0, baseline_goals=0.55), seed=2022)
    ...
    groups_2022 = {
        ...
        "Group H": ["Portugal", "Korea Republic", "Uruguay", "Ghana"]
    }

    # Build TeamRatings for all 32 teams using the 2022 edition dataset
    ratings = {}
    for g_name, team_list in groups_2022.items():
        for t in team_list:
            key_match = None
            for k in oracle.players_by_year_team.get(2022, oracle.players_by_year_team[2021]):
            ...
            try:
                ...
                pool, yr = oracle._get_player_pool(key_match, 2021)
    ...
    all_teams = [t for group in groups_2022.values() for t in group]
    pair_probs = {}
    ...
    # 1. Single Tournament Realization (Seed 2022)
    group_results = {}
    group_qualifiers = {}
...
if __name__ == "__main__":
    res = run_2022_world_cup_simulation()
    
    print("\n" + "="*75)
    print("[2022 FIFA WORLD CUP (QATAR) - MODEL SIMULATION RESULTS]")
    print("="*75)
